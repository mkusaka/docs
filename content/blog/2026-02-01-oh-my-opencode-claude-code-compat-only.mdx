...
| `agents.X.disable` | 個別エージェントを無効化（スキーマの`disabled_agents`に未登録のエージェントもこちらで無効化可能） |
| `git_master.commit_footer` | コミットメッセージへの「Ultraworked with Sisyphus」フッター付与を無効化 |
| `git_master.include_co_authored_by` | コミットメッセージへの`Co-authored-by: Sisyphus`トレーラー付与を無効化 |
