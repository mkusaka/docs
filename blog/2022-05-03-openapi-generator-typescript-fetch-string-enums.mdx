...
- 今回、enum ではなく typescript のオブジェクトへ変換することもできるオプションが追加されたので、enum に対して懸念がある場合はこちらも検討してみると良いよ(実際にはデフォルトでオブジェクトへ変換されるため、enum を使いたい場合はオプションを true 似すると良いよ)
...
個人的のもこの点はどうにかしたいと思っていたため、OpenAPI Generator 本体に UnionType として出力できるオプションを追加する[Pull Request](https://github.com/OpenAPITools/openapi-generator/pull/11976)をマージしてもらいました。
