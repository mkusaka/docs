...
function CodeBlock({ node, children, ...props }: ComponentPropsWithoutRef<"pre"> & { node?: unknown }) {
  ...
  return (
    <div className="group relative my-4">
      {highlightedHtml ? (
        <div
          className="[&_pre]:text-[0.8125rem] [&_pre]:leading-[1.7] [&_pre]:!bg-secondary [&_pre]:border [&_pre]:border-border [&_pre]:rounded-xl [&_pre]:p-4 [&_pre]:overflow-x-auto [&_pre]:m-0"
          dangerouslySetInnerHTML={{ __html: highlightedHtml }}
        />
      ) : (
        <pre
          {...props}
          className="text-[0.8125rem] leading-[1.7] text-foreground bg-secondary border border-border rounded-xl p-4 overflow-x-auto"
        >
          {children}
        </pre>
      )}
      <button
        onClick={handleCopy}
        className="absolute right-2 top-2 p-1.5 rounded-lg border border-border bg-secondary text-muted-foreground hover:text-foreground hover:bg-accent transition-all opacity-0 group-hover:opacity-100 cursor-pointer"
        title="Copy code"
      >
        {copied ? (
